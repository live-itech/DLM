@php
    $canSeeCost = auth()->user()->can('reports.finance');
    $productsJson = $products->map(fn ($p) => [
        'id' => $p->id, 'name' => $p->code . ' — ' . $p->name,
        'price' => $canSeeCost ? (float) $p->cost_price : 0, 'unit' => $p->unit?->symbol ?? '',
    ])->values();
    $itemsJson = $order->exists ? $order->items->map(fn ($i) => [
        'product_id' => $i->product_id, 'qty' => (float) $i->qty, 'price' => (float) $i->cost_price,
        'discount' => (float) $i->discount,
    ])->values() : collect();
@endphp
<x-app-layout>
    <x-slot name="header">{{ $order->exists ? 'Edit' : 'Buat' }} Purchase Order</x-slot>

    <x-page-header :title="($order->exists ? 'Edit' : 'Buat') . ' Purchase Order'" subtitle="Pesanan pembelian ke supplier" />

    <form method="POST" action="{{ $order->exists ? route('purchase-orders.update', $order) : route('purchase-orders.store') }}"
          x-data="orderForm({ products: {{ Illuminate\Support\Js::from($productsJson) }}, items: {{ Illuminate\Support\Js::from($itemsJson) }}, priceField: 'cost_price', isTaxable: {{ old('is_taxable', $order->is_taxable) ? 'true' : 'false' }}, ppnRate: {{ old('ppn_rate', $order->ppn_rate ?: 11) }}, discount: {{ old('discount', $order->discount ?: 0) }}, withReason: false })">
        @csrf
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            <thead class="bg-gray-50"><tr>
                                ...
                                <th class="table-th text-right">Harga Beli</th><th class="table-th text-right">Diskon</th>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <template x-for="(row, i) in items" :key="i">
                                    <tr>
                                        ...
                                        <td class="px-3 py-2 align-top">
                                            <input type="number" step="0.01" min="0" :name="`items[${i}][cost_price]`" x-model.number="row.price" class="form-input w-28 text-right" required>
                                        </td>
                                        <td class="px-3 py-2 align-top">
            ...
            <div class="space-y-5">
                <div class="card space-y-4">
                    ...
                    <div class="flex flex-col gap-2 pt-2">
                        ...
                        <a href="{{ route('purchase-orders.index') }}" class="btn-outline w-full">Batal</a>
                    </div>
                    <p class="text-xs text-gray-400">PO tersimpan sebagai draft. Stok bertambah saat penerimaan barang.</p>
                </div>
            </div>
        </div>
    </form>
